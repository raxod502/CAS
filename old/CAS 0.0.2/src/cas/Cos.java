...
public class Cos extends SingleArgumentFunction {
	...
	public Expr simplifyFunction() {
		...
		if (getArgument() instanceof ArcSin) {
			...
			return new Sqrt(new Difference(ONE, new DualExponentiation(arg.getArgument(), TWO))).simplify();
		}
		...
		if (getArgument() instanceof ArcTan) {
			...
			return new Sqrt(new DualSum(ONE, new DualExponentiation(arg.getArgument(), TWO))).inverse().simplify();
		}
		if (getArgument() instanceof Negative) {
			return new Cos(((Negative)getArgument()).getArgument()).simplify();
		}
	...
	public static boolean isDivisible(Number arg, Number div) {
		return ((Number)new Difference(arg, div).simplify()).isDivisibleBy(new Multiple(TWO, pi));
	}
	public Expr derivativePartial(int var) {
		return new Sin(getArgument()).negative().simplify();
	}
	...
	public Compare isReal() {
		Expr re = new Re(getArgument()).simplify(), im = new Im(getArgument()).simplify();
	...
	public Compare isImag() {
		...
		Expr re = new Re(getArgument()).simplify();
		if (re instanceof Number) {
