...
public class Log10 extends SingleArgumentFunction {
	...
	public Expr simplifyFunction() {
		...
		if (getArgument() instanceof ComplexExpr) {
			return new Quotient(new Log(getArgument()), new Log(TEN)).simplify();
		}
		// Difference
		// DualExponentiation
		// log(x^y) = y log x
		if (getArgument() instanceof DualExponentiation) {
			return new DualProduct(((DualExponentiation)getArgument()).getPower(), new Log10(((DualExponentiation)getArgument()).getBase())).simplify();
		}
		// DualProduct
		// log xy = log x + log y
		if (getArgument() instanceof DualProduct) {
			return new DualSum(new Log10(((DualProduct)getArgument()).getLeft()), new Log10(((DualProduct)getArgument()).getRight())).simplify();
		}
		// DualSum
		// LogBase
		// Quotient
		...
		if (getArgument() instanceof Quotient) {
			return new Difference(new Log10(((Quotient)getArgument()).getNumerator()), new Log10(((Quotient)getArgument()).getDenominator())).simplify();
		}
		// Exponentiation
		// log(x^y^z) = yz log x
		...
		if (getArgument() instanceof Sqrt) {
			return new DualProduct(oneHalf, new Log10(((Sqrt)getArgument()).getArgument())).simplify();
		}
		// Tan
		// Complex
		// reduce to natural logs
		...
		if (getArgument() instanceof Exponent) {
			return new DualProduct(((Exponent)getArgument()).power, new Log10(((Exponent)getArgument()).base)).simplify();
		}
		// Fraction
		// log(a/b) = log a - log b
		if (getArgument() instanceof Fraction) {
			return new Difference(new Log10(((Fraction)getArgument()).numerator), new Log10(((Fraction)getArgument()).denominator)).simplify();
		}
		// Imaginary
		// reduce to natural logs
		...
		if (getArgument() instanceof PolynomialTerm) {
			...
			return new DualSum(new Log10(term.coefficient), new DualProduct(term.power, new Log10(term.variable))).simplify();
		}
		// Variable
	...
	public Expr derivativePartial(int var) {
		...
		return new DualProduct(getArgument(), new Log(TEN)).inverse().simplify();
	}
