...
public class LogBase extends DoubleArgumentFunction {
	...
	public Expr derivative(Variable var) {
		return new Quotient(new Log(getArgument()), new Log(getBase())).derivative(var).simplify();
	}
	public Expr derivativePartial(int var) {
		Expr b = getBase(), c = getArgument();
		if (var == 0)
			return new Quotient(new Log(c), new DualProduct(b, new Log(b).square())).negative().simplify();
		if (var == 1)
			return new DualProduct(c, new Log(b)).inverse().simplify();
		return ZERO;
	}
