...
public abstract class Expr {
	...
	public final Expr simplify(HashSet<Expr> equivalenceClass) {
		Expr.addExprToLists(this, expr -> expr.simplifyExpr(), equivalenceClass, false);
		return this;
	}
	static final void addExprToLists(Expr thisExpr, Function<Expr, Expr> simplifyFunction, HashSet<Expr> equivalenceClass, boolean forceReduction) {
		Collection<HashSet<Expr>> exprGroups = InformationPasser.exprGroups;
		
		// Get equivalence class containing this expr
		HashSet<Expr> thisGroup;
		if (equivalenceClass != null) {
			thisGroup = equivalenceClass;
		}
		else {
			thisGroup = InformationPasser.getContainingGroup(thisExpr);
		}
