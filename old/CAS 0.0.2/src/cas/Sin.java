...
public class Sin extends SingleArgumentFunction {
	...
	public Expr simplifyFunction() {
		...
		if (getArgument() instanceof ArcTan) {
			...
			return new Quotient(arg.getArgument(),
					new Sqrt(new DualSum(new DualExponentiation(arg.getArgument(), TWO), ONE))).simplify();
		}
		if (getArgument() instanceof Negative) {
			return new Sin(((Negative)getArgument()).getArgument()).negative().simplify();
		}
	...
	public Expr derivativePartial(int var) {
		return new Cos(getArgument()).simplify();
	}
