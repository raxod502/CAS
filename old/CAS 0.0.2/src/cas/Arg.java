...
public class Arg extends SingleArgumentFunction {
	...
	public Expr simplifyFunction() {
		...
		return new ImaginaryExpr(new Log(new Sgn(getArgument()))).negative().simplify();
	}
	public Expr derivativePartial(int var) {
		if (var != 0) return ZERO;
		return new Arg(getArgument(), 1).simplify();
	}
