...
public class FunctionConstant extends NumIntegerTranscendentalFunctionConstant {
	...
	public double doubleValue() {
		...
		if (func instanceof Arg) {
			return Math.atan2(((NumNumSum)new Im(func.getArgument(0)).simplify()).doubleValue(),
					((NumNumSum)new Re(func.getArgument(0)).simplify()).doubleValue());
		}
		if (func instanceof Cis) {
//			return new ComplexDouble(((NumNumSum)new Im(func.getArgument(0)).simplify()).doubleValue(),
//					((NumNumSum)new Re(func.getArgument(0)).simplify()).doubleValue());
		}
...
//			return new ComplexDouble(func.getDouble(0), func.getDouble(1));
		...
		if (func instanceof Im) {
			return ((NumNumSum)func.simplify()).doubleValue();
		}
