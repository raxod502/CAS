	...
	public Expr simplifyFunction() {
		...
		if (getNumerator() instanceof Exponent && getDenominator() instanceof Exponent) {
			...
			if (num.base.equalTo(den.base)) {
				return new Exponent(num.base, (NumFraction)new Difference(num.power, den.power).simplify(null)).simplify();
			}
		}
		// Fraction
		// (a/b)/x = a/(b*x)
		if (getNumerator() instanceof Fraction) {
			...
			return new Fraction(num.numerator, (NumExponent)new DualProduct(num.denominator, getDenominator()).simplify(null)).simplify();
		}
		// x/(a/b) = (x*b)/a
		if (getDenominator() instanceof Fraction) {
			Fraction den = (Fraction)getDenominator();
			return new Fraction((NumExponent)new DualProduct(getNumerator(), den.denominator).simplify(null), den.numerator).simplify();
		}
		// FunctionConstant
		// Imaginary
