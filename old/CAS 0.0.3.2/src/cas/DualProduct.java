...
public class DualProduct extends DualFunction {
	...
	public Expr simplifyDualFunction() {
		...
		if (getRight() instanceof ComplexExpr) {
			...
			return new ComplexExpr(new DualProduct(getLeft(), arg.getReal()), new DualProduct(getLeft(), arg.getImag())).simplify();
		}
		// Difference
		// x * (a-b) = x*a - x*b
		if (getRight() instanceof Difference) {
			Difference diff = (Difference)getRight();
			return new Difference(new DualProduct(getLeft(), diff.getLeft()), new DualProduct(getLeft(), diff.getRight())).simplify();
		}
		// DualExponentiation
		// x^a * x^b = x^(a+b)
		if (getRight() instanceof DualExponentiation &&
			...
			return new DualExponentiation(left.getBase(), new DualSum(left.getPower(), right.getPower())).simplify();
		}
		// DualProduct
		// a*(b*c) = a*b*c
		if (getRight() instanceof DualProduct) {
			DualProduct arg = (DualProduct)getRight();
			return new Product(getLeft(), arg.getLeft(), arg.getRight()).simplify();
		}
		// DualSum
		// x * (a+b) = x*a + x*b
		if (getRight() instanceof DualSum) {
			DualSum arg = (DualSum)getRight();
			return new DualSum(new DualProduct(getLeft(), arg.getLeft()), new DualProduct(getLeft(), arg.getRight())).simplify();
		}
		// LogBase
		// Quotient
		// x * (y/z) = (x*y)/z
		if (getRight() instanceof Quotient) {
			...
			return new Quotient(new DualProduct(getLeft(), arg.getNumerator()), arg.getDenominator()).simplify();
		}
		// Exponentiation
		// Arg
		// ArcCos
		...
		if (getRight() instanceof ImaginaryExpr) {
			...
			return new ImaginaryExpr(new DualProduct(getLeft(), right.getArgument())).simplify();
		}
		// Inverse
		// a * 1/b = a/b
		if (getRight() instanceof Inverse) {
			return new Quotient(getLeft(), ((Inverse)getRight()).getArgument()).simplify();
		}
		// Log
		// Log10
		// Negative
		...
		if (getRight() instanceof Negative) {
			return new DualProduct(getLeft(), ((Negative)getRight()).getArgument()).negative().simplify();
		}
		// Re
		// Sin
		// Sgn
		...
		if (getRight() instanceof Sqrt && getLeft() instanceof Sqrt) {
			return new Sqrt(new DualProduct(((Sqrt)getLeft()).getArgument(), ((Sqrt)getRight()).getArgument())).simplify();
		}
		// Tan
		// Complex
		// x * (a + bi) = x*a + (x*b)i
		if (getRight() instanceof Complex) {
			...
			return new ComplexExpr(new DualProduct(getLeft(), arg.real), new DualProduct(getLeft(), arg.imag)).simplify();
		}
		// Exponent
		// x^a * x^b = x^(a+b)
		if (getRight() instanceof Exponent &&
			...
			return new Exponent(left.base, (NumIntegerTranscendentalFunctionConstant)new DualSum(left.power, right.power).simplify()).simplify();
		}
		// Fraction
		// x * y/z = (x*y)/z
		if (getRight() instanceof Fraction) {
			Fraction arg = (Fraction)getRight();
			return new Quotient(new DualProduct(getLeft(), arg.numerator), arg.denominator).simplify();
		}
		// FunctionConstant
		// Imaginary
		if (getRight() instanceof Imaginary) {
			Imaginary right = (Imaginary)getRight();
			return new ImaginaryExpr(new DualProduct(getLeft(), right.value)).simplify();
		}
		// Integer
		...
		if (getRight() instanceof PolynomialTerm) {
			...
			if (getLeft() instanceof PolynomialTerm && poly.variable.equalTo(((PolynomialTerm)getLeft()).variable)) {
				...
				return new PolynomialTerm((NumIntegerTranscendentalFunctionConstant)new DualProduct(poly.coefficient, polyL.coefficient).simplify(), poly.variable, (Integer)new DualSum(poly.power, polyL.power).simplify()).simplify();
			}
			return new Product(getLeft(), poly.coefficient, new DualExponentiation(poly.variable, poly.power)).simplify();
		}
		// Variable
		
		if (getLeft() instanceof Number && getRight() instanceof Number) {
			return new Multiple((NumExponent)getLeft(), (NumExponent)getRight()).reduce();
		}
		return this;
	}
