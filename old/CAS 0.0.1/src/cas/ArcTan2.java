...
public class ArcTan2 extends DoubleArgumentFunction {
	...
	public Expr simplifyFunction() {
		...
		Expr negRootThree = rootThree.negative().simplify();
		// atan(0, 0) = undef
		...
		// atan(-x, +x) = 3π/4
		if (left.negative().simplify().equalTo(right) && left.isNegative() == YES) {
			return new Multiple(threeFourths, pi);
		}
		// atan(+x, -x) = -π/4
		if (right.negative().simplify().equalTo(left) && right.isNegative() == YES) {
		...
		// atan(+x√3, +x) = π/6
		if (new DualProduct(right, rootThree).simplify().equalTo(left) && right.isPositive() == YES) {
			return new Multiple(oneSixth, pi);
		}
		// atan(+x, +x√3) = π/3
		if (new DualProduct(left, rootThree).simplify().equalTo(right) && left.isPositive() == YES) {
			return new Multiple(oneThird, pi);
		}
		// atan(-x, +x√3) = 2π/3
		if (new DualProduct(left, negRootThree).simplify().equalTo(right) && left.isNegative() == YES) {
			return new Multiple(twoThirds, pi);
		}
		// atan(-x√3, +x) = 5π/6
		if (new DualProduct(right, negRootThree).simplify().equalTo(left) && right.isPositive() == YES) {
			return new Multiple(fiveSixths, pi);
		}
		// atan(-x√3, -x) = -5π/6
		if (new DualProduct(right, rootThree).simplify().equalTo(left) && right.isNegative() == YES) {
			return new Multiple(negFiveSixths, pi);
		}
		// atan(-x, -x√3) = -2π/3
		if (new DualProduct(left, rootThree).simplify().equalTo(right) && left.isNegative() == YES) {
			return new Multiple(negTwoThirds, pi);
		}
		// atan(+x, -x√3) = -π/3
		if (new DualProduct(left, negRootThree).simplify().equalTo(right) && left.isPositive() == YES) {
			return new Multiple(negOneThird, pi);
		}
		// atan(+x√3, -x) = -π/6
		if (new DualProduct(right, negRootThree).simplify().equalTo(left) && right.isNegative() == YES) {
	...
	public Expr derivative(Variable var) {
		...
		return new Quotient(new Difference(new DualProduct(x, y.derivative(var)), new DualProduct(x.derivative(var), y)),
				new DualSum(new DualExponentiation(x, TWO), new DualExponentiation(y, TWO))).simplify();
	}
	public Expr derivativePartial(int var) {
		Expr x = getLeft(), y = getRight();
		if (var == 0) {
			return new Quotient(y.negative(), new DualSum(x.square(), y.square())).simplify();
		}
		if (var == 1) {
			return new Quotient(x, new DualSum(x.square(), y.square())).simplify();
		}
		return ZERO;
	}
