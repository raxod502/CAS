...
public class DualSum extends DualFunction {
	...
	public Expr simplifyDualFunction() {
		...
		// (-x) + x = 0
		if (getLeft().negative().simplify().equalTo(getRight())) {
			return ZERO;
		}
		// x + x = 2*x
		if (getLeft().equalTo(getRight())) {
			return new DualProduct(TWO, getLeft()).simplify();
		}
		...
		if ((getLeft() instanceof Product || getLeft() instanceof DualProduct) &&
			...
			Function left = new Product(new ArrayList<Expr>(((Function)getLeft()).args));
			Function right = new Product(new ArrayList<Expr>(((Function)getRight()).args));
		...
		if (getLeft() instanceof Difference) {
			...
			return new Sum(left.getLeft(), left.getRight().negative(), getRight()).simplify();
		}
		// DualExponentiation
		...
		if (getLeft() instanceof DualSum) {
			...
			return new Sum(left.getLeft(), left.getRight(), getRight()).simplify();
		}
		// LogBase
		...
		if (getLeft() instanceof LogBase && getRight() instanceof LogBase &&
			...
			return new LogBase(left.getBase(), new DualProduct(left.getArgument(), right.getArgument())).simplify();
		}
		// Quotient
		// a/c + b/c = (a+b)/c
		// a/b + c/d = (ad+bc)/(bd)
		if (getLeft() instanceof Quotient && getRight() instanceof Quotient) {
			...
			if (left.getDenominator().equalTo(right.getDenominator())) {
				return new Quotient(new DualSum(left.getNumerator(), right.getNumerator()), left.getDenominator()).simplify();
			}
			else {
				Expr a = left.getNumerator(), b = left.getDenominator(), c = right.getNumerator(), d = right.getDenominator();
				return new Quotient(new DualSum(new DualProduct(a, d), new DualProduct(b, c)), new DualProduct(b, d)).simplify();
			}
		}
		// a/b + c = (a + bc)/b
		if (getLeft() instanceof Quotient) {
			...
			return new Quotient(new DualSum(left.getNumerator(),
					new DualProduct(left.getDenominator(), getRight())), left.getDenominator()).simplify();
		}
		// Exponentiation
		// Arg
		// ArcCos
		...
		if (getLeft() instanceof ImaginaryExpr) {
			return new ComplexExpr(getRight(), ((ImaginaryExpr)getLeft()).getArgument()).simplify();
		}
		// Inverse
		// 1/b + c = (1+bc)/b
		if (getLeft() instanceof Inverse) {
			...
			return new Quotient(new DualSum(ONE,
					new DualProduct(left.getArgument(), getRight())), left.getArgument()).simplify();
		}
		// Log
		if (getLeft() instanceof Log && getRight() instanceof Log) {
			Log left = (Log)getLeft(), right = (Log)getRight();
			return new Log(new DualProduct(left.getArgument(), right.getArgument())).simplify();
		}
		// Log10
		if (getLeft() instanceof Log10 && getRight() instanceof Log10) {
			Log10 left = (Log10)getLeft(), right = (Log10)getRight();
			return new Log10(new DualProduct(left.getArgument(), right.getArgument())).simplify();
		}
		// Negative
		if (getLeft() instanceof Negative) {
			return new Difference(getRight(), ((Negative)getLeft()).getArgument()).simplify();
		}
		// Re
		// Sin
		// Sgn
		...
		if (getLeft() instanceof Complex) {
			...
			return new ComplexExpr(new DualSum(left.real, getRight()), left.imag).simplify();
		}
		// Exponent
		// Fraction
		// a/b + c = (a+bc)/b
		if (getLeft() instanceof Fraction) {
			...
			return new Quotient(new DualSum(left.numerator,
					new DualProduct(left.denominator, getRight())), left.denominator).simplify();
		}
		// FunctionConstant
		// Imaginary
		// bi + c = c + bi
		if (getLeft() instanceof Imaginary) {
			return new ComplexExpr(getRight(), ((Imaginary)getLeft()).value).simplify();
		}
		// Integer
		...
		if (getLeft() instanceof PolynomialTerm && getRight() instanceof PolynomialTerm) {
			...
			if (left.variable.equalTo(right.variable) && left.power.equalTo(right.power)) {
				return new PolynomialTerm((NumIntegerTranscendentalFunctionConstant)new DualSum(left.coefficient, right.coefficient).simplify(), left.variable, left.power);
			}
		}
		// Variable
