...
public class Cis extends SingleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		// Cis[x] = Cos[x] + i Sin[x]
		return new ComplexExpr(new Cos(getArgument()), new Sin(getArgument())).simplify(eq);
	}
	public Expr derivativePartial(int var) {
		if (var != 0) return ZERO;
		return new ComplexExpr(new Sin(getArgument()).negative(), new Cos(getArgument())).simplify(null);
	}
