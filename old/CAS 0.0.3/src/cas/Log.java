	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		...
		if (getArgument() instanceof ComplexExpr) {
			...
			return new ComplexExpr(new Log(new Sqrt(new Sum(new DualExponentiation(real, TWO),
					...
					new Arg(getArgument())).simplify(eq);
		}
		// Difference
		// DualExponentiation
		// log(x^y) = y log x
		if (getArgument() instanceof DualExponentiation) {
			return new DualProduct(((DualExponentiation)getArgument()).getPower(), new Log(((DualExponentiation)getArgument()).getBase())).simplify(eq);
		}
		// DualProduct
		// log xy = log x + log y
		if (getArgument() instanceof DualProduct) {
			return new DualSum(new Log(((DualProduct)getArgument()).getLeft()), new Log(((DualProduct)getArgument()).getRight())).simplify(eq);
		}
		// DualSum
		// LogBase
		// Quotient
		...
		if (getArgument() instanceof Quotient) {
			return new Difference(new Log(((Quotient)getArgument()).getNumerator()), new Log(((Quotient)getArgument()).getDenominator())).simplify(eq);
		}
		// Exponentiation
		// log(x^y^z) = yz log x
		...
		if (getArgument() instanceof ImaginaryExpr) {
			...
			return new ComplexExpr(new Log(b), new DualProduct(new Sgn(b), new Multiple(oneHalf, pi))).simplify(eq);
		}
		// Inverse
		// log(1/x) = -log x
		...
		if (getArgument() instanceof Sqrt) {
			return new DualProduct(oneHalf, new Log(((Sqrt)getArgument()).getArgument())).simplify(eq);
		}
		// Tan
		// Complex
		// log(a + bi) = log √(a^2+b^2) + i atan2(a, b)
		...
		if (getArgument() instanceof Exponent) {
			return new DualProduct(((Exponent)getArgument()).power, new Log(((Exponent)getArgument()).base)).simplify(eq);
		}
		// Fraction
		// log(a/b) = log a - log b
		if (getArgument() instanceof Fraction) {
			return new Difference(new Log(((Fraction)getArgument()).numerator), new Log(((Fraction)getArgument()).denominator)).simplify(eq);
		}
		// Imaginary
		// log(bi) = log b + i sgn(b) π/2
		...
		if (getArgument() instanceof PolynomialTerm) {
			...
			return new DualSum(new Log(term.coefficient), new DualProduct(term.power, new Log(term.variable))).simplify(eq);
		}
		// Variable
