...
public class ComplexExpr extends DoubleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		...
		if (getReal().isZero() == YES) {
			return new ImaginaryExpr(getImag()).simplify(eq);
		}
		// a + 0i = a
		...
		if (getReal() instanceof Imaginary) {
			return new ImaginaryExpr(new DualSum(getReal(), getImag())).simplify(eq);
		}
		// a + (bi)i = (a-b)
		if (getImag() instanceof Imaginary) {
			return new Difference(getReal(), getImag()).simplify(eq);
		}
		// (c + di) + bi = c + (b+d)i
		if (getReal() instanceof Complex) {
			return new ComplexExpr(((Complex)getReal()).real, new DualSum(((Complex)getReal()).imag, getImag())).simplify(eq);
		}
		// (c + di) + bi = c + (b+d)i
		if (getReal() instanceof ComplexExpr) {
			return new ComplexExpr(((ComplexExpr)getReal()).getReal(), new DualSum(((ComplexExpr)getReal()).getImag(), getImag())).simplify(eq);
		}
		// a + (c + di)i = (a+c) - di
		if (getImag() instanceof Complex) {
			Complex arg = (Complex)getImag();
			return new ComplexExpr(new DualSum(getReal(), arg.real), arg.imag.negative()).simplify(eq);
		}
		// a + (c + di)i = (a+c) - di
		if (getImag() instanceof ComplexExpr) {
			ComplexExpr arg = (ComplexExpr)getImag();
			return new ComplexExpr(new DualSum(getReal(), arg.getReal()), arg.getImag().negative()).simplify(eq);
		}
		// Cannot simplify
	...
	public Expr conjugate(HashSet<Expr> eq) {
