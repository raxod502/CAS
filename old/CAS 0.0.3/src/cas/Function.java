...
public abstract class Function extends Expr {
	...
	public final Expr simplifyExpr(HashSet<Expr> eq) {
		...
		for (Expr arg : args) {
			...
			if (containingGroup == null) {
				Expr.addExprToLists(arg, (expr, eqc) -> expr.simplifyExpr(eqc), containingGroup, false);
		...
		for (List<Expr> argPermutation : argPermutations) {
			try {
				...
				if (this instanceof SingleArgumentFunction) {
					...
					instance = getClass().getDeclaredConstructor(ArrayList.class).newInstance(argPermutation);
				}
				HashSet<Expr> equivalenceClass = InformationPasser.getContainingGroup(this);
				if (canSimplifyFunction) {
					result = Expr.addExprToLists(instance, (function, eqc) -> {
						if (function instanceof Function) {
							return ((Function)function).simplifyFunction(eqc);
						}
						else {
					...
					Expr.addExprToLists(instance, (function, eqc) -> function,
	...
	public abstract Expr simplifyFunction(HashSet<Expr> eq);
	...
	public Expr derivative(Variable var) {
		...
		if (ders.size() == 1) return ders.get(0).simplify(null);
		if (ders.size() == 2) return new DualSum(ders.get(0), ders.get(1)).simplify(null);
		return new Sum(ders).simplify(null);
	}
