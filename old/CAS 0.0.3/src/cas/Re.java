...
public class Re extends SingleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		if (getArgument() instanceof ComplexExpr) {
			return new DualSum(new Re(((ComplexExpr)getArgument()).getReal()), new Im(((ComplexExpr)getArgument()).getImag())).simplify(eq);
		}
		...
		if (getArgument() instanceof ImaginaryExpr) {
			return new Im(((ImaginaryExpr)getArgument()).getArgument()).simplify(eq);
		}
		...
		if (getArgument() instanceof Cis) {
			...
			return new Cos(arg.getArgument()).simplify(eq);
		}
