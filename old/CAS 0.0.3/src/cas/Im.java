...
public class Im extends SingleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		if (getArgument() instanceof ComplexExpr) {
			return new DualSum(new Im(((ComplexExpr)getArgument()).getReal()), new Re(((ComplexExpr)getArgument()).getImag())).simplify(eq);
		}
		...
		if (getArgument() instanceof ImaginaryExpr) {
			return new Re(((ImaginaryExpr)getArgument()).getArgument()).simplify(eq);
		}
