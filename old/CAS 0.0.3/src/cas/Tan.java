...
public class Tan extends SingleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		...
		if (getArgument() instanceof ArcSin) {
			...
			return new Quotient(arg.getArgument(), new Sqrt(
					new Difference(ONE, new DualExponentiation(arg.getArgument(), TWO)))).simplify(eq);
		}
		if (getArgument() instanceof ArcCos) {
			ArcCos arg = (ArcCos)getArgument();
			return new Quotient(new Sqrt(new Difference(ONE, new DualExponentiation(arg.getArgument(), TWO))), arg.getArgument()).simplify(eq);
		}
		...
		if (getArgument() instanceof Negative) {
			return new Tan(((Negative)getArgument()).getArgument()).negative().simplify(eq);
		}
	...
	public static boolean isDivisible(Number arg, Number div) {
		return ((Number)new Difference(arg, div).simplify(null)).isDivisibleBy(pi);
	}
	public Expr derivativePartial(int var) {
		return new DualExponentiation(new Cos(getArgument()), TWO).inverse().simplify(null);
	}
