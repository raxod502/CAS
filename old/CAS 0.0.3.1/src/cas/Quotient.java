...
public class Quotient extends DoubleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		...
		if (getNumerator().equalTo(ONE)) {
			return new Inverse(getDenominator()).simplify(eq);
		}
		// 0/x = 0
		...
		if (getNumerator().negative().simplify(null).equalTo(getDenominator())) {
			return NEG_ONE;
		}
			...
			for (Expr arg : originalArgs) {
				if (arg.equalTo(getNumerator())) {
					...
					if (newArgs.size() == 0) {
						...
						return new Inverse(newArgs.get(0)).simplify(eq);
					}
					else if (newArgs.size() == 2) {
						return new Inverse(new DualProduct(newArgs.get(0), newArgs.get(1))).simplify(eq);
					}
					else {
						return new Inverse(new Product(newArgs)).simplify(eq);
					}
				}
			}
		}
			...
			for (Expr arg : originalArgs){
				if (arg.equalTo(getDenominator())) {
					...
					if (newArgs.size() == 0) {
						...
						return newArgs.get(0).simplify(eq);
					}
					else if (newArgs.size() == 2) {
						return new DualProduct(newArgs.get(0), newArgs.get(1)).simplify(eq);
					}
					else {
		...
		if ((getNumerator() instanceof Product || getNumerator() instanceof DualProduct ||
			...
			if (numDotArgs.size() != numArgs.size() && denDotArgs.size() != denArgs.size()) {
				...
				return new Quotient(newNum, newDen).simplify(eq);
			}
		}
		...
		if (getDenominator() instanceof ComplexExpr) {
			...
			return new Quotient(new DualProduct(getNumerator(), den.conjugate(eq)), new DualSum(
					new DualExponentiation(den.getReal(), TWO), new DualExponentiation(den.getImag(), TWO))).simplify(eq);
		}
		// (a+bi)/x = [a/x] + [b/x]i
		if (getNumerator() instanceof ComplexExpr) {
			ComplexExpr num = (ComplexExpr)getNumerator();
			return new ComplexExpr(new Quotient(num.getReal(), getDenominator()), new Quotient(num.getImag(), getDenominator())).simplify(eq);
		}
		// Difference
		// DualExponentiation
		// x^a/x^b = x^(a-b)
		if (getNumerator() instanceof DualExponentiation && getDenominator() instanceof DualExponentiation) {
			...
			if (num.getBase().equalTo(den.getBase())) {
				return new DualExponentiation(num.getBase(), new Difference(num.getPower(), den.getPower())).simplify(eq);
			}
		}
		...
		if (getNumerator() instanceof LogBase && getDenominator() instanceof LogBase) {
			...
			if (num.getBase().equalTo(den.getBase())) {
				return new LogBase(den.getBase(), num.getBase()).simplify(eq);
			}
		}
		// Quotient
		// (a/b)/x = a/(b*x)
		if (getNumerator() instanceof Quotient) {
			...
			return new Quotient(num.getNumerator(), new DualProduct(num.getDenominator(), getDenominator())).simplify(eq);
		}
		// x/(a/b) = (x*b)/a
		if (getDenominator() instanceof Quotient) {
			Quotient den = (Quotient)getDenominator();
			return new Quotient(new DualProduct(getNumerator(), den.getDenominator()), den.getNumerator()).simplify(eq);
		}
		// Exponentiation
		// x^a^b/x^a^c = x^a^(b-c)
		...
		if (getNumerator() instanceof Exponentiation && getDenominator() instanceof Exponentiation) {
			...
			if (success) {
				...
				return new Exponentiation(powers).simplify(eq);
			}
		}
		// Arg
		// ArcCos
		...
		if (getNumerator() instanceof Cis && getDenominator() instanceof Cis) {
			...
			return new Cis(new Difference(num.getArgument(), den.getArgument())).simplify(eq);
		}
		// Cos - see Tan
		// Exp
		// (e^y)/(e^x) = e^(y-x)
		if (getNumerator() instanceof Exp && getDenominator() instanceof Exp) {
			...
			return new Exp(new Difference(num.getArgument(), den.getArgument())).simplify(eq);
		}
		// Im
		// ImaginaryExpr
		// a/(bi) = (-ai)/b
		if (getNumerator() instanceof ImaginaryExpr) {
			...
			return new ImaginaryExpr(new Quotient(num.getArgument(), getDenominator())).simplify(eq);
		}
		if (getDenominator() instanceof ImaginaryExpr) {
			ImaginaryExpr den = (ImaginaryExpr)getDenominator();
			return new Quotient(new Negative(new ImaginaryExpr(getNumerator())), den.getArgument()).simplify(eq);
		}
		// Inverse
		// (1/a)/b = 1/(ab)
		if (getNumerator() instanceof Inverse) {
			Inverse num = (Inverse)getNumerator();
			return new Inverse(new DualProduct(num.getArgument(), getDenominator())).simplify(eq);
		}
		// a/(1/b) = a/b
		if (getDenominator() instanceof Inverse) {
			Inverse den = (Inverse)getDenominator();
			return new Quotient(getNumerator(), den.getArgument()).simplify(eq);
		}
		// Log
		...
		if (getNumerator() instanceof Log && getDenominator() instanceof Log) {
			...
			return new LogBase(den.getArgument(), num.getArgument()).simplify(eq);
		}
		// Log10
		...
		if (getNumerator() instanceof Negative) {
			return new Negative(new Quotient(((Negative)getNumerator()).getArgument(), getDenominator())).simplify(eq);
		}
		if (getDenominator() instanceof Negative) {
			return new Negative(new Quotient(getNumerator(), ((Negative)getDenominator()).getArgument())).simplify(eq);
		}
		// Re
		// Sin
		// Sgn
		...
		if (getDenominator() instanceof Complex) {
			...
			return new Quotient(new DualProduct(getNumerator(), den.conjugate(eq)), new DualSum(
					new DualExponentiation(den.real, TWO), new DualExponentiation(den.imag, TWO))).simplify(eq);
		}
		// Exponent
		// x^a/x^b = x^(a-b)
		if (getNumerator() instanceof Exponent && getDenominator() instanceof Exponent) {
			...
			if (num.base.equalTo(den.base)) {
				return new Exponent(num.base, (NumFraction)new Difference(num.power, den.power).simplify(null)).simplify(eq);
			}
		}
		// Fraction
		// (a/b)/x = a/(b*x)
		if (getNumerator() instanceof Fraction) {
			...
			return new Fraction(num.numerator, (NumExponent)new DualProduct(num.denominator, getDenominator()).simplify(null)).simplify(eq);
		}
		// x/(a/b) = (x*b)/a
		if (getDenominator() instanceof Fraction) {
			Fraction den = (Fraction)getDenominator();
			return new Fraction((NumExponent)new DualProduct(getNumerator(), den.denominator).simplify(null), den.numerator).simplify(eq);
		}
		// FunctionConstant
		// Imaginary
		if (getNumerator() instanceof Imaginary) {
			Imaginary num = (Imaginary)getNumerator();
			return new ImaginaryExpr(new Quotient(num.value, getDenominator())).simplify(eq);
		}
		if (getDenominator() instanceof Imaginary) {
			Imaginary den = (Imaginary)getDenominator();
			return new Quotient(new Negative(new ImaginaryExpr(getNumerator())), den.value).simplify(eq);
		}
		// Integer
		if (getNumerator() instanceof Integer && getDenominator() instanceof Integer) {
			...
			if (reduced) {
				...
				if (denValue == 1) {
					...
					return new Quotient(new Integer(numValue), new Integer(denValue)).simplify(eq);
				}
			}
			else {
		...
		if (getNumerator() instanceof DualSum) {
			...
			return new DualSum(new Quotient(num.getLeft(), getDenominator()), new Quotient(num.getRight(), getDenominator())).simplify(eq);
		}
		if (getNumerator() instanceof Sum) {
			...
			return new Sum(fractions).simplify(eq);
		}
		...
		if (getNumerator() instanceof Number && getDenominator() instanceof Number) {
			return new Fraction((NumIntegerTranscendentalFunctionConstant)getNumerator(),
					(NumIntegerTranscendentalFunctionConstant)getDenominator()).reduce(eq);
		}
		return this;
	}
	...
	public Expr derivativePartial(int var) {
		if (var == 0) return getDenominator().inverse().simplify(null);
		if (var == 1) return new Quotient(getNumerator(), getDenominator().square()).negative().simplify(null);
		return ZERO;
	}
