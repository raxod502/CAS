...
public class LogBase extends DoubleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		...
		if (getBase().equalTo(new Transcendental(Transcendent.E))) {
			return new Log(getArgument()).simplify(eq);
		}
		if (getBase().equalTo(TEN)) {
			return new Log10(getArgument()).simplify(eq);
		}
		...
		if (getArgument() instanceof ComplexExpr) {
			return new Quotient(new Log(getArgument()), new Log(getBase())).simplify(eq);
		}
		// Difference
		// DualExponentiation
		...
		if (getArgument() instanceof DualExponentiation) {
			...
			return new DualProduct(arg.getPower(), new LogBase(getBase(), arg.getBase())).simplify(eq);
		}
		// DualProduct
		// logbase(b, xy) = logbase(b, x) + logbase(b, y)
		if (getArgument() instanceof DualProduct) {
			DualProduct arg = (DualProduct)getArgument();
			return new DualSum(new LogBase(getBase(), arg.getLeft()), new LogBase(getBase(), arg.getRight())).simplify(eq);
		}
		// DualSum
		// LogBase
		// Quotient
		...
		if (getArgument() instanceof Quotient) {
			...
			return new Difference(new LogBase(getBase(), arg.getNumerator()), new LogBase(getBase(), arg.getDenominator())).simplify(eq);
		}
		// Exponentiation
		...
		if (getArgument() instanceof Exp) {
			...
			return new DualProduct(arg.getArgument(), new LogBase(getBase(), new Transcendental(Transcendent.E))).simplify(eq);
		}
		// Im
		// ImaginaryExpr
		...
		if (getArgument() instanceof Inverse) {
			...
			return new LogBase(getBase(), arg.getArgument()).negative().simplify(eq);
		}
		// Log
		// Log10
		// Negative
		...
		if (getArgument() instanceof Negative) {
			...
			return new DualSum(new LogBase(getBase(), NEG_ONE), new LogBase(getBase(), arg.getArgument())).simplify(eq);
		}
		// Re
		// Sin
		// Sgn
		...
		if (getArgument() instanceof Sqrt) {
			...
			return new DualProduct(TWO.inverse(), new LogBase(getBase(), arg.getArgument())).simplify(eq);
		}
		// Tan
		// Complex
		// logbase(c, a+bi) = log(a+bi)/log(c)
		...
		if (getArgument() instanceof Exponent) {
			...
			return new DualProduct(arg.power, new LogBase(getBase(), arg.base)).simplify(eq);
		}
		// Fraction
		// logbase(b, x/y) = logbase(b, x) - logbase(b, y)
		if (getArgument() instanceof Fraction) {
			Fraction arg = (Fraction)getArgument();
			return new Difference(new LogBase(getBase(), arg.numerator), new LogBase(getBase(), arg.denominator)).simplify(eq);
		}
		// FunctionConstant
		// Integer
		...
		if (getArgument() instanceof Integer && getBase() instanceof Integer) {
			...
			if (!failure) {
				return new Quotient(new Integer(argFactorsCount), new Integer(baseFactorsCount)).simplify(eq);
			}
		}
		...
		if (getBase() instanceof DualExponentiation) {
			...
			return new DualProduct(new Inverse(base.getPower()), new LogBase(base.getBase(), getArgument())).simplify(eq);
		}
		// DualProduct
		// logbase(x/y, c)
		// DualSum
		...
		if (getBase() instanceof Exponentiation) {
			...
			newArgs.remove(0);
			return new DualProduct(new Product(newArgs).inverse(), new LogBase(base.args.get(0), getArgument())).simplify(eq);
		}
		// Arg
		// ArcCos
		// ArcSin
		...
		if (getBase() instanceof Exp) {
			...
			return new Quotient(new Log(getArgument()), base.getArgument()).simplify(eq);
		}
		// Im
		// ImaginaryExpr
		...
		if (getBase() instanceof Inverse) {
			...
			return new LogBase(base.getArgument(), new Inverse(getArgument())).simplify(eq);
		}
		// Log
		// Log10
		// Negative
		...
		if (getBase() instanceof Sqrt) {
			...
			return new DualProduct(TWO, new LogBase(base.getArgument(), getArgument())).simplify(eq);
		}
		// Tan
		// Complex
		...
		if (getBase() instanceof Exponent) {
			...
			return new DualProduct(new Inverse(base.power), new LogBase(base.base, getArgument())).simplify(eq);
		}
		// Fraction
		// FunctionConstant
		// logbase(x/y, z)
