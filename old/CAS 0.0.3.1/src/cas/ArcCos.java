...
public class ArcCos extends SingleArgumentFunction {
	...
	public Expr derivativePartial(int var) {
		if (var == 0) return new Sqrt(new Difference(ONE, getArgument().square())).inverse().negative().simplify(null);
		return ZERO;
	}
	...
	public Compare isImag() {
		...
		return new Re(getArgument()).simplify(null).greaterThanOrEqualTo(ONE);
	}
