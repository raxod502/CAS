...
public class Exp extends SingleArgumentFunction {
	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		...
		if (getArgument() instanceof ComplexExpr) {
			...
			return new DualProduct(new Exp(arg.getReal()), new Exp(new ImaginaryExpr(arg.getImag()))).simplify(eq);
		}
		// Difference
		// e^(a-b) = e^a / e^b
		if (getArgument() instanceof Difference) {
			Difference arg = (Difference)getArgument();
			return new Quotient(new Exp(arg.getLeft()), new Exp(arg.getRight())).simplify(eq);
		}
		// DualExponentiation
		// e^(a^b)
		// DualProduct
		...
		if (getArgument() instanceof DualProduct) {
			...
			return new Exponentiation(E, arg.getLeft(), arg.getRight()).simplify(eq);
		}
		// DualSum
		// e^(x+y) = e^x * e^y
		if (getArgument() instanceof DualSum) {
			DualSum arg = (DualSum)getArgument();
			return new DualProduct(new Exp(arg.getLeft()), new Exp(arg.getRight())).simplify(eq);
		}
		// LogBase
		// e^logbase(b, c) = e^(ln[b]/ln[c]) = e^(ln[b])^(1/ln[c]) = b^(1/ln[c])
		if (getArgument() instanceof LogBase) {
			LogBase arg = (LogBase)getArgument();
			return new DualExponentiation(arg.getBase(), new Log(arg.getArgument()).inverse()).simplify(eq);
		}
		// Quotient
		// e^(b/c) = e^b^(1/c)
		if (getArgument() instanceof Quotient) {
			Quotient arg = (Quotient)getArgument();
			return new Exponentiation(E, arg.getNumerator(), arg.getDenominator().inverse()).simplify(eq);
		}
		// Exponentiation
		// e^(x^y^z)
		// Arg
		...
		if (getArgument() instanceof ImaginaryExpr) {
			return new Cis(((ImaginaryExpr)getArgument()).getArgument()).simplify(eq);
		}
		// Inverse
		// Log
		// e^log(x) = x
		...
		if (getArgument() instanceof Log10) {
			return new DualExponentiation(((Log10)getArgument()).getArgument(), new Log(TEN).inverse()).simplify(eq);
		}
		// Negative
		// e^(-x) = 1/e^x
		if (getArgument() instanceof Negative) {
			Negative arg = (Negative)getArgument();
			return new Exp(arg.getArgument()).inverse().simplify(eq);
		}
		// Re
		// Sin
		// Sgn
		...
		if (getArgument() instanceof Complex) {
			...
			return new DualProduct(new Exp(arg.real), new Exp(new Imaginary(arg.imag))).simplify(eq);
		}
		// Exponent
		// Fraction
		// e^(x/y) = e^x^(1/y)
		if (getArgument() instanceof Fraction) {
			...
			return new Exponentiation(E, arg.numerator, arg.denominator.inverse()).simplify(eq);
		}
		// FunctionConstant
		// Imaginary
		// e^(xi) = cis(x)
		if (getArgument() instanceof Imaginary) {
			return new Cis(((Imaginary)getArgument()).value).simplify(eq);
		}
		// Integer
		// Multiple
		// e^(xyz) = e^x^y^z
		...
		if (getArgument() instanceof PolynomialTerm) {
			...
			return new Exponentiation(E, arg.coefficient, new DualExponentiation(arg.variable, arg.power)).simplify(eq);
		}
		// Variable
	...
	public Compare isPositive() {
		...
		Expr imPart = new Im(getArgument()).simplify(null);
		Compare cond2 = MAYBE;
