	...
	public Expr simplifyFunction(HashSet<Expr> eq) {
		...
		if (left.isReal() == YES && right.isReal() == YES) {
			...
			if (left.isPositive() == YES) {
				return new ArcTan(new Quotient(right, left)).simplify(eq);
			}
			// atan(x, y) = atan(y/x) + π
			if (left.isNegative() == YES && right.isNegative() == NO) {
				return new DualSum(new ArcTan(new Quotient(right, left)), pi).simplify(eq);
			}
			// atan(x, y) = atan(y/x) - π
			if (left.isNegative() == YES && right.isNegative() == YES) {
				return new Difference(new ArcTan(new Quotient(right, left)), pi).simplify(eq);
			}
		}
