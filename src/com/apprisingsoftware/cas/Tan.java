	...
	@Override public Expr getAlternate() {
		...
		if (getArgument() instanceof Number) {
			...
			if (isDivisible(arg, new Multiple(oneHalf, pi)))
				throw new AssertionError("The tangent of π/2 is undefined (positive ∞).");
			// tan(2π/3) = -√3
		...
		if (getArgument() instanceof ArcSin) {
			...
			return new Quotient(arg.getArgument(), new Sqrt(
					new Difference(ONE, new DualExponentiation(arg.getArgument(), TWO))));
		}
		if (getArgument() instanceof ArcCos) {
			ArcCos arg = (ArcCos)getArgument();
			return new Quotient(new Sqrt(new Difference(ONE, new DualExponentiation(arg.getArgument(), TWO))), arg.getArgument());
		}
		...
		if (getArgument() instanceof Negative) {
			return new Tan(((Negative)getArgument()).getArgument()).negative();
		}
//		if (getArgument() instanceof Number) return new FunctionConstant(this);
	...
	public static boolean isDivisible(Number arg, Number div) {
		return ((Number)new Difference(arg, div).getSimplest()).isDivisibleBy(pi);
	}
	@Override public Expr derivativePartial(int var) {
		return new DualExponentiation(new Cos(getArgument()), TWO).inverse().getSimplest();
	}
