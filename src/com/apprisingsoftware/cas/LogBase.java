...
class LogBase extends DoubleArgumentFunction {
	...
	@Override public Expr getAlternate() {
		...
		if (getBase().equalTo(new Transcendental(Transcendent.E))) {
			return new Log(getArgument());
		}
		if (getBase().equalTo(TEN)) {
			return new Log10(getArgument());
		}
		...
		if (getArgument() instanceof ComplexExpr) {
			return new Quotient(new Log(getArgument()), new Log(getBase()));
		}
		// Difference
		// DualExponentiation
		...
		if (getArgument() instanceof DualExponentiation) {
			...
			return new DualProduct(arg.getPower(), new LogBase(getBase(), arg.getBase()));
		}
		// DualProduct
		// logbase(b, xy) = logbase(b, x) + logbase(b, y)
		if (getArgument() instanceof DualProduct) {
			DualProduct arg = (DualProduct)getArgument();
			return new DualSum(new LogBase(getBase(), arg.getLeft()), new LogBase(getBase(), arg.getRight()));
		}
		// DualSum
		// LogBase
		// Quotient
		...
		if (getArgument() instanceof Quotient) {
			...
			return new Difference(new LogBase(getBase(), arg.getNumerator()), new LogBase(getBase(), arg.getDenominator()));
		}
		// Exponentiation
		...
		if (getArgument() instanceof Exp) {
			...
			return new DualProduct(arg.getArgument(), new LogBase(getBase(), new Transcendental(Transcendent.E)));
		}
		// Im
		// ImaginaryExpr
		...
		if (getArgument() instanceof Inverse) {
			...
			return new LogBase(getBase(), arg.getArgument()).negative();
		}
		// Log
		// Log10
		// Negative
		...
		if (getArgument() instanceof Negative) {
			...
			return new DualSum(new LogBase(getBase(), NEG_ONE), new LogBase(getBase(), arg.getArgument()));
		}
		// Re
		// Sin
		// Sgn
		...
		if (getArgument() instanceof Sqrt) {
			...
			return new DualProduct(TWO.inverse(), new LogBase(getBase(), arg.getArgument()));
		}
		// Tan
		// Complex
		// logbase(c, a+bi) = log(a+bi)/log(c)
		...
		if (getArgument() instanceof Exponent) {
			...
			return new DualProduct(arg.power, new LogBase(getBase(), arg.base));
		}
		// Fraction
		// logbase(b, x/y) = logbase(b, x) - logbase(b, y)
		if (getArgument() instanceof Fraction) {
			Fraction arg = (Fraction)getArgument();
			return new Difference(new LogBase(getBase(), arg.numerator), new LogBase(getBase(), arg.denominator));
		}
		// FunctionConstant
		// Integer
		...
		if (getArgument() instanceof Integer && getBase() instanceof Integer) {
			...
			if (!failure) {
				return new Quotient(new Integer(argFactorsCount), new Integer(baseFactorsCount));
			}
		}
		...
		if (getBase() instanceof DualExponentiation) {
			...
			return new DualProduct(base.getPower().inverse(), new LogBase(base.getBase(), getArgument()));
		}
		// DualProduct
		// logbase(x/y, c)
		// DualSum
		...
		if (getBase() instanceof Exponentiation) {
			...
			newArgs.remove(0);
			return new DualProduct(new Product(newArgs).inverse(), new LogBase(base.args.get(0), getArgument()));
		}
		// Arg
		// ArcCos
		// ArcSin
		...
		if (getBase() instanceof Exp) {
			...
			return new Quotient(new Log(getArgument()), base.getArgument());
		}
		// Im
		// ImaginaryExpr
		...
		if (getBase() instanceof Inverse) {
			...
			return new LogBase(base.getArgument(), getArgument().inverse());
		}
		// Log
		// Log10
		// Negative
		...
		if (getBase() instanceof Sqrt) {
			...
			return new DualProduct(TWO, new LogBase(base.getArgument(), getArgument()));
		}
		// Tan
		// Complex
		...
		if (getBase() instanceof Exponent) {
			...
			return new DualProduct(base.power.inverse(), new LogBase(base.base, getArgument()));
		}
		// Fraction
		// FunctionConstant
		// logbase(x/y, z)
...
//			return new FunctionConstant(this);
	...
	@Override public Expr derivative(Variable var) {
		return new Quotient(new Log(getArgument()), new Log(getBase())).derivative(var).getSimplest();
	}
	@Override public Expr derivativePartial(int var) {
		Expr b = getBase(), c = getArgument();
		if (var == 0)
			return new Quotient(new Log(c), new DualProduct(b, new Log(b).square())).negative().getSimplest();
		if (var == 1)
			return new DualProduct(c, new Log(b)).inverse().getSimplest();
		return ZERO;
	}
