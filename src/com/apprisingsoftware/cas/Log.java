...
class Log extends SingleArgumentFunction {
	...
	@Override public Expr getAlternate() {
		...
		if (getArgument() instanceof DualExponentiation) {
			return new DualProduct(((DualExponentiation)getArgument()).getPower(), new Log(((DualExponentiation)getArgument()).getBase()));
		}
		// DualProduct
		// log xy = log x + log y
		if (getArgument() instanceof DualProduct) {
			return new DualSum(new Log(((DualProduct)getArgument()).getLeft()), new Log(((DualProduct)getArgument()).getRight()));
		}
		// DualSum
		// LogBase
		// Quotient
		...
		if (getArgument() instanceof Quotient) {
			return new Difference(new Log(((Quotient)getArgument()).getNumerator()), new Log(((Quotient)getArgument()).getDenominator()));
		}
		// Exponentiation
		// log(x^y^z) = yz log x
		...
		if (getArgument() instanceof ImaginaryExpr) {
			...
			return new ComplexExpr(new Log(b), new DualProduct(new Sgn(b), new Multiple(oneHalf, pi)));
		}
		// Inverse
		// log(1/x) = -log x
		if (getArgument() instanceof Inverse) {
			return new Log(((Inverse)getArgument()).getArgument()).negative();
		}
		// Log
		// Log10
		// Negative
		...
		if (getArgument() instanceof Sqrt) {
			return new DualProduct(oneHalf, new Log(((Sqrt)getArgument()).getArgument()));
		}
		// Tan
		// Complex
		// log(a + bi) = log √(a^2+b^2) + i atan2(a, b)
		...
		if (getArgument() instanceof Exponent) {
			return new DualProduct(((Exponent)getArgument()).power, new Log(((Exponent)getArgument()).base));
		}
		// Fraction
		// log(a/b) = log a - log b
		if (getArgument() instanceof Fraction) {
			return new Difference(new Log(((Fraction)getArgument()).numerator), new Log(((Fraction)getArgument()).denominator));
		}
		// Imaginary
		// log(bi) = log b + i sgn(b) π/2
		...
		if (getArgument() instanceof PolynomialTerm) {
			...
			return new DualSum(new Log(term.coefficient), new DualProduct(term.power, new Log(term.variable)));
		}
		// Variable
