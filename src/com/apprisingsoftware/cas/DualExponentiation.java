...
class DualExponentiation extends DualFunction {
	...
	@Override public Expr getAlternateWithoutCommutivity() {
		...
		if (getBase().isZero() == YES && getPower().isZero() == YES) {
			throw new AssertionError("0^0 is undefined.");
		}
		// x^0 = 1
		...
		if (getPower() instanceof ComplexExpr) {
			...
			return new DualProduct(new DualExponentiation(getBase(), pow.getReal()),
					new DualExponentiation(getBase(), new ImaginaryExpr(pow.getImag())));
		}
		// Difference
		// x^(a-b) = x^a / x^b
		if (getPower() instanceof Difference) {
			Difference pow = (Difference)getPower();
			return new Quotient(new DualExponentiation(getBase(), pow.getLeft()), new DualExponentiation(getBase(), pow.getRight()));
		}
		// DualExponentiation
		// x^(a^b)
		// DualProduct
		...
		if (getPower() instanceof DualProduct) {
			...
			return new Exponentiation(getBase(), pow.getLeft(), pow.getRight());
		}
		// DualSum
		// x^(a+b) = x^a * x^b
		if (getPower() instanceof DualSum) {
			DualSum pow = (DualSum)getPower();
			return new DualProduct(new DualExponentiation(getBase(), pow.getLeft()), new DualExponentiation(getBase(), pow.getRight()));
		}
		// LogBase
		if (getPower() instanceof LogBase) {
			// x^logbase(x, y) = y
			if (getBase().equalTo(((LogBase)getPower()).getBase())) {
				return ((LogBase)getPower()).getArgument();
			}
		}
		// Quotient
		// x^(a/b) = x^a^(1/b)
		if (getPower() instanceof Quotient) {
			...
			if (!pow.getNumerator().equalTo(ONE)) {
				return new Exponentiation(getBase(), pow.getNumerator(), pow.getDenominator()).inverse();
			}
		}
		// Exponentiation
		// x^(a^b^c)
		...
		if (getPower() instanceof ImaginaryExpr) {
			...
			return new DualProduct(new Exp(new DualProduct(d.negative(), new Arg(a))), new Cis(new DualProduct(d, new Log(a))));
		}
		// Inverse
		// x^(1/y)
		// Log
		if (getPower() instanceof Log) {
			...
			if (getBase().equalTo(new Transcendental(Transcendent.E))) {
				return ((Log)getPower()).getArgument();
			}
		}
		// Log10
		if (getPower() instanceof Log10) {
			...
			if (getBase().equalTo(TEN)) {
				return ((Log10)getPower()).getArgument();
			}
		}
		// Negative
		// x^(-y) = 1/(x^y)
		if (getPower() instanceof Negative) {
			...
			return new DualExponentiation(getBase(), pow.getArgument()).inverse();
		}
		// Re
		// Sin
		// Sgn
		...
		if (getPower() instanceof Complex) {
			...
			return new DualProduct(new DualExponentiation(getBase(), pow.real), new DualExponentiation(getBase(), new Imaginary(pow.imag)));
		}
		// Exponent
		// x^(a^b)
		// Fraction
		...
		if (getPower() instanceof Fraction) {
			...
			return new Exponentiation(getBase(), pow.numerator, pow.denominator).inverse();
		}
		// FunctionConstant
		// Imaginary
		// a^(di) = e^(-d*arg[a]) * cis(d ln a)
		...
		if (getPower() instanceof Integer) {
			...
			if (pow.isNegative() == YES) {
				return new DualExponentiation(getBase(), pow.negative()).inverse();
			}
		}
		// Multiple
		// x^(a*b*c) = x^a^b^c [ = ((x^a)^b)^c ]
		...
		if (getPower() instanceof PolynomialTerm) {
			...
			return new Exponentiation(getBase(), pow.coefficient, new DualExponentiation(pow.variable, pow.power));
		}
		// Variable
		...
		if (getBase() instanceof ComplexExpr) {
			...
			return new DualProduct(firstTerm, secondTerm);
		}
		// Difference
		// (a-b)^x
		// DualExponentiation
		...
		if (getBase() instanceof DualProduct) {
			return new DualProduct(new DualExponentiation(getBase(), ((DualProduct)getBase()).getLeft()),
					new DualExponentiation(getBase(), ((DualProduct)getBase()).getRight()));
		}
		// DualSum
		// (a+b)^x
		// LogBase
		...
		if (getBase() instanceof Quotient) {
			return new Quotient(new DualExponentiation(((Quotient)getBase()).getNumerator(), getPower()),
					new DualExponentiation(((Quotient)getBase()).getDenominator(), getPower()));
		}
		// Exponentiation
		// (a^b^c)^x = a^b^c^x
		if (getBase() instanceof Exponentiation) {
			...
			terms.add(getPower());
			return new Exponentiation(terms);
		}
		// Arg
		// ArcCos
		// ArcSin
		...
		if (getBase() instanceof Cis) {
			return new Cis(new DualProduct(((Cis)getBase()).getArgument(), getPower()));
		}
		// Cos
		// Exp
		// (e^x)^y = e^x^y
		if (getBase() instanceof Exp) {
			...
			return new Exponentiation(new Transcendental(Transcendent.E), base.getArgument(), getPower());
		}
		// Im
		// ImaginaryExpr
		// (a+bi)^(c+di) = (a^2+b^2)^(c/2) * e^(-d*arg(a+bi)) * cis(c*arg(a+bi) + d/2 ln(a^2+b^2))
		...
		if (getBase() instanceof ImaginaryExpr) {
			...
			Expr c = getPower();
			return new DualProduct(new DualExponentiation(b, c), new Cis(new DualProduct(c, new Arg(new ImaginaryExpr(b)))));
		}
		// Inverse
		// (1/a)^b = 1/a^b
		if (getBase() instanceof Inverse) {
			Inverse base = (Inverse)getBase();
			return new DualExponentiation(base.getArgument(), getPower()).inverse();
		}
		// Log
		// Log10
		// Negative
		...
		if (getBase() instanceof Negative) {
			return new DualProduct(new DualExponentiation(NEG_ONE, getPower()),
					new DualExponentiation(((Negative)getBase()).getArgument(), getPower()));
		}
		// Re
		// Sin
		// Sgn
		...
		if (getBase() instanceof Sqrt) {
			...
			return new Exponentiation(base.getArgument(), TWO.inverse(), getPower());
		}
		// Tan
		// Complex
		// (a+bi)^(c+di) = (a^2+b^2)^(c/2) * e^(-d*arg(a+bi)) * cis(c*arg(a+bi) + d/2 ln(a^2+b^2))
		...
		if (getBase() instanceof Fraction) {
			return new Quotient(new DualExponentiation(((Fraction)getBase()).numerator, getPower()),
					new DualExponentiation(((Fraction)getBase()).denominator, getPower()));
		}
		// FunctionConstant
		// Imaginary
		// (bi)^c = b^c * cis(c*arg(bi))
		...
		if (getBase() instanceof PolynomialTerm) {
			...
			return new DualProduct(new DualExponentiation(poly.coefficient, getPower()), new Exponentiation(poly.variable, poly.power, getPower()));
		}
		// Variable
	...
	@Override public Expr derivativePartial(int var) {
		if (var == 0) return new DualProduct(getPower(), new DualExponentiation(getBase(), new Difference(getPower(), ONE))).getSimplest();
		if (var == 1) return new DualProduct(new DualExponentiation(getBase(), getPower()), new Log(getBase())).getSimplest();
		return ZERO;
	}
